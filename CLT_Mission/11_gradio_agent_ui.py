...
2. **Dual-Execution Engine**:
   - **Live DeepSeek Agent**: Runs Sridhar's unified `create_agent()` live over our suite of 5 custom 
...
with gr.Blocks(css=custom_css, title="Capstone Agent Dashboard") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=5):
            ...
            with gr.Row():
                gr.Markdown(
                    "**💡 Try these sample prompts in the Simulator Mode:**\n"
                    "- *'Which products in the database have a price greater than $200?'* (SQLite tool)\n"
                    "- *'I am traveling to Tokyo. Check if it's raining and tell me what to bring!'* (API tool chaining)\n"
                    "- *'What is the final invoice for 12 desk chairs at $249.99 each with a 15% discount and 8% tax?'* (Pricing tool)"
                )
