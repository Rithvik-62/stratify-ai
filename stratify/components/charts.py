...
Interactive Plotly Charts Component (charts.py) - Enhanced Visualizations & Historical Comparisons
"""
...
def render_historical_comparison_panel(hist_comp):
    ...
    comp_df = pd.DataFrame([
        {"Period": "Prior Period", "Revenue": prior_rev, "Profit": prior_prof},
...
def render_revenue_performance_chart(sales_df):
    ...
    with col_m:
        metric_choice = st.radio("SELECT METRIC", ["Revenue", "Profit", "Quantity"], horizontal=True, key="rev_perf_metric")
    with col_f:
        time_choice = st.radio("TIME HORIZON", ["7D", "30D", "90D", "ALL"], horizontal=True, key="rev_perf_time")
    ...
    color_map = {"Revenue": "#6366f1", "Profit": "#10b981", "Quantity": "#38bdf8"}
...
def render_product_margin_bubble_chart(sales_df):
    ...
    fig_bubble = px.scatter(
        ...
        hover_name=prod_col, template="plotly_dark",
...
def render_top_products_ranking(products_df, sales_df):
    """Renders Top Products ranking (#1 to #5) with horizontal bar charts & metrics."""
